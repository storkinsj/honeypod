...
import honeypod #AddedForHoneyPod
...
blacklist = set()
whitelist = set()
honeylog = '/var/log/honeypod'  #AddedForHoneyPod

intercept_address = os.system("ip addr show") #AddedForHoneyPod
...
def init(id, cfg):
    ...
    if "honeylog" in os.environ:  #AddedForHoneyPod
        global honeylog = os.getenv('honeylog')
    return True
...
#        if (check_name(name, blacklist)):    #AddedForHoneyPod blacklist/nonwhitelist same.
        else:   #AddedForHoneyPod
#            log_info("dns_filter.py: "+name+" blacklisted")
            os.system('echo -e "HONEYPOD: Suspicious hostname resolution! "' + name + '" > ' + honeylog)
